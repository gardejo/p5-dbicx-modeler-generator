...
So, I recommend that to separate schemata and models with L<DBICx::Modeler>.
This smart module provides a L<Moose>-based model layer over L<DBIx::Class>.
...
Dynamic generation of L<DBIx::Class> schema modules

=item *

Dynamic generation of L<DBICx::Modeler> model modules

=back

As well, this C<DBICx::Modeler::Generator> module
can also be used as a simple automation wrapper
only dynamic generation of schema modules with L<DBIx::Class::Schema::Loadler>.
...
It catches L<DBICx::Modeler>'s test modules
...
for additional definition.
For example, L<Moose>'s attributes, methods, method modifiers, and much more.
...
You may use also the wrapped interface with L<DBICx::Modeler::Generator::CLI>.

=head3 C<< DBICx::Modeler::Generator::CLI->new_with_options(%init_args) >>

Returns an object of L<DBICx::Modeler::Generator::CLI>.
...
This class and subclasses using L<MooseX::Orochi> for DI (dependency injection).
...
It is an instance of an implement class
...
with the C<perldocjp> command from L<Pod::PerldocJp>.
