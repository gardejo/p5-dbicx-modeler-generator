...
そこで、L<DBICx::Modeler>を使ってスキーマとモデルのモジュールを分離することを推奨します。
この小気味よいモジュールは、L<DBIx::Class>スキーマに対して、L<Moose>に基づいたレイヤーを提供します。
...
L<DBIx::Class>スキーマモジュール群の動的な定義

=item *

L<DBICx::Modeler>モデルモジュール群の動的な定義

=back

なお、このC<DBICx::Modeler::Generator>モジュールは、L<DBIx::Class::Schema::Loadler>によるスキーマモジュール群の動的な生成を自動化する、単純なラッパーとしても使えます。
...
これはL<DBICx::Modeler>のテスト用モジュール群を再現したもので、C<artist>, C<cd>, C<track>というテーブル(スキーマ)を備えています。

=item 2.

文書化のためのB<任意の作業として>、I<MySQL Workbench>を使ってER図(ERD: Entity-Relationship Diagram)を描画します。

ER図として出力した画像ファイルの実例(サンプル)は、このディストリビューションのF<examples/doc/DBDERII_Including Information.png>に同梱されています。
...
追加の定義とは、L<Moose>のアトリビュート(attributes), メソッド(methods), メソッドモディファイヤー(method modifiers)などです。
...
L<DBICx::Modeler::Generator::CLI>によって依存性の注入を包み込んだインターフェースを使うことも出来ます。

=head3 C<< DBICx::Modeler::Generator::CLI->new_with_options(%init_args) >>

L<DBICx::Modeler::Generator::CLI>オブジェクトを返します。

ジェネレーターオブジェクトを取得するためには、このインターフェースを使うことをB<強く>推奨します。L<MooseX::Getopt>とL<MooseX::SimpleConfig>を用いて依存性の注入作業を包み込むことが出来るためです。
...
このクラスとそのサブクラスでは依存性の注入(DI: Dependency Injection)のためにL<MooseX::Orochi>を使っています。
...
L<DBICx::Modeler::Generator::ClassLike>インターフェースを満たす実装クラスのインスタンスです。

このディストリビューションでは、一般的な用途に使える実装クラスとして、L<DBICx::Modeler::Generator::Driver::Class>を同梱しています。
...
L<DBICx::Modeler::Generator::DriverLike>インターフェースを満たす実装クラスのインスタンスです。

このディストリビューションでは、一般的な用途に使える実装クラスとして、L<DBICx::Modeler::Generator::Driver::MySQL>とL<DBICx::Modeler::Generator::Driver::SQLite>を同梱しています。

=head3 C<< /DBICx/Modeler/Generator/Model >>

L<DBICx::Modeler::Generator::ModelLike>インターフェースを満たす実装クラスのインスタンスです。

このディストリビューションでは、一般的な用途に使える実装クラスとして、L<DBICx::Modeler::Generator::Driver::Model>を同梱しています。

=head3 C<< /DBICx/Modeler/Generator/Path >>

L<DBICx::Modeler::Generator::PathLike>インターフェースを満たす実装クラスのインスタンスです。

このディストリビューションでは、一般的な用途に使える実装クラスとして、L<DBICx::Modeler::Generator::Driver::Path>を同梱しています。
...
L<Path::Class::Dir> (L<MooseX::Types::Path::Class>で型変換されます)
...
L<DBICx::Modeler::Generator::SchemaLike>インターフェースを満たす実装クラスのインスタンスです。

このディストリビューションでは、一般的な用途に使える実装クラスとして、L<DBICx::Modeler::Generator::Driver::Schema>を同梱しています。

=head3 C<< /DBICx/Modeler/Generator/Tree >>

L<DBICx::Modeler::Generator::TreeLike>インターフェースを満たす実装クラスのインスタンスです。

このディストリビューションでは、一般的な用途に使える実装クラスとして、L<DBICx::Modeler::Generator::Driver::Tree>を同梱しています。
...
C<< mysql >>
(ドライバーの実装クラスがL<DBICx::Modeler::Generator::Driver::MySQL>の場合),
C<< sqlite3 >>
(ドライバーの実装クラスがL<DBICx::Modeler::Generator::Driver::SQLite>の場合),
等
...
L<Path::Class::File> (L<MooseX::Types::Path::Class>で型変換されます)
...
I<MySQL Workbench>アプリケーションの日本語版ホームページ,
L<http://www-jp.mysql.com/products/workbench/>

訳注: このアプリケーションの日本語版が存在するわけではありません。
...
何かバグを発見されたら、機能のご要望がありましたら、または改善のためのご意見がありましたら、L<http://github.com/gardejo/p5-dbicx-modeler-generator/issues>のバグトラッカーを使って報告してください。
...
日本語版はL<Pod::PerldocJp>を使ったC<perldocjp>コマンドで閲覧出来ます。
...
テストのコード網羅率(コードカバレッジ)を検査するために、L<Devel::Cover>を使っています。
...
詳しくはL<perlgpl>およびL<perlartistic>を参照してください。
...
例えば、「constructor」を「コンストラクタ」ではなく「コンストラクタB<ー>」と表記しています。
